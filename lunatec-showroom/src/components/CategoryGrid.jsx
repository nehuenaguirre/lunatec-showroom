  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4 mb-10">
      ...
      <div ref={scrollRef} className="flex gap-4 md:gap-6 overflow-x-auto pb-6 pt-2 px-2 [&::-webkit-scrollbar]:hidden scroll-smooth">
        ...
        {/* Botón Todas */}
        <Link to="/" className={`flex flex-col items-center gap-3 min-w-[85px] group transition-all ${isAllActive ? 'scale-105' : 'opacity-80'}`}>
          ...
          <span className={`text-[11px] font-bold uppercase tracking-wide ${isAllActive ? 'text-brand-dark' : 'text-gray-500'}`}>Todas</span>
        </Link>

        {/* Categorías con Imagen Inteligente */}
        {categorias.map((cat) => {
          const isActive = location.pathname === `/categoria/${cat.id}`;
          return (
            <Link key={cat.id} to={`/categoria/${cat.id}`} className={`flex flex-col items-center gap-3 min-w-[85px] group transition-all ${isActive ? 'scale-105' : 'opacity-80'}`}>
